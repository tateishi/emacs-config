...
(defun my/create-fontset-from-list (fonts name)
  "FONTSとNAMEからfontsetをつくる."
  (message "%s:%s" fonts name)
  (if font-list
      (let ((res (my/create-fontset (car fonts) name)))
        (if res
            res
          (my/create-fontset-from-list (cdr fonts) name)))))
